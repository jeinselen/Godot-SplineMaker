...
- Undo/redo suppressed while trigger or grip active on either controller
- Use native OS features where available (file picker, text entry, etc.); do not reimplement platform UI
- Mesh preview must update in realtime during all editing operations
...
- Draw mode point placement requires a smart algorithm (open source preferred, custom if needed) that produces smooth NURBS curves from hand-drawn paths
